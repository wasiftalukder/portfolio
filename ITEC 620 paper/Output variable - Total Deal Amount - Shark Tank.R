...
# Inputs
y <- 'Total.Deal.Amount' # input y column name
training_size <- 0.6
set.seed(12345)
...
#MLR Interaction
MLRInteractionmodel <- lm(Total.Deal.Amount ~ . + Original.Ask.Amount*Original.Offered.Equity -Got.Deal -Valuation.Requested, data=training)
summary(MLRInteractionmodel)

# With only the significant independent variables
MLRInteractionmodel <- lm(Total.Deal.Amount ~ Original.Ask.Amount + Original.Offered.Equity + Total.Deal.Equity + Number.of.sharks.in.deal + Loan + Industry_Media.Entertainment + Industry_Travel + Original.Ask.Amount*Original.Offered.Equity, data=training)
summary(MLRInteractionmodel)
...
tree <- tree(Total.Deal.Amount ~ . -Number.of.sharks.in.deal -Got.Deal -Valuation.Requested, data=training)
